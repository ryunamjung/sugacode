...
**주의사항**  
이 시스템은 병원 데이터를 필터링하기 위한 도구입니다.  
엑셀 파일에는 `병원등급`, `제외`, `종양여부`, `이식`, `진료과1~6`, `EDI코드`, `명칭`, `산정명칭` 컬럼이 반드시 포함되어야 하며,  
`병원등급`은 `/`로 구분된 복수 항목일 수 있습니다.  
B등급과 C등급은 모두 선택 해제할 경우, 해당 등급에 아무 항목도 포함되지 않은 데이터만 포함되며,  
빈값은 기본적으로 포함 처리됩니다.  
