...
본 시스템은 표준수가코드를 필터링하기 위한 도구입니다. 업로드한 엑셀에는 <code>병원등급</code>, <code>제외</code>, <code>종양여부</code>, <code>이식</code>, <code>진료과1~6</code>, <code>EDI코드</code>, <code>명칭</code>, <code>산정명칭</code> 컬럼이 포함되어야 합니다.<br>
<code>병원등급</code>은 여러 값이 <code>/</code>로 구분되어 있을 수 있으며, 빈값도 포함됩니다.<br>
B등급과 C등급 항목은 기본적으로 전체 선택되어 있으며, 사용자가 제외 항목을 해제할 수 있습니다.<br>
결과는 선택된 조건에 따라 필터링되어 최종 리스트로 출력됩니다.<br>
필터는 위에서 아래 순서대로 적용되며, 각 단계의 선택 여부에 따라 결과가 달라질 수 있습니다.<br>
업로드할 엑셀 파일에는 암진료 및 이식 여부 데이터가 <code>O</code>로 표시되어야 합니다.
</div>
