...
※ ‘병원등급’에는 슬래시(/)로 구분된 여러 항목이 들어갈 수 있습니다.  
※ B·C 등급 모두를 선택 해제한 경우, 해당 등급 항목이 하나도 없는 데이터만 결과에 포함됩니다.  
※ 빈값은 자동으로 포함되며, 필터는 위에서 아래로 순차 적용됩니다. 
※ 데이터를 넣을때 표기방식은 동일하게 해주세요.
