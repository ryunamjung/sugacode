...
본 시스템은 표준수가코드를 필터링하기 위한 도구입니다. 업로드한 엑셀에는 `병원등급`, `제외`, `종양여부`, `이식`, `진료과1~6`, `EDI코드`, `명칭`, `산정명칭` 컬럼이 포함되어야 합니다.<br>
`병원등급`은 여러 값이 `/`로 구분되어 있을 수 있으며, 빈값도 포함됩니다.<br>
B등급, C등급 항목은 기본적으로 전체 선택되어 있으며, 사용자가 제외 항목을 선택할 수 있습니다.<br>
결과는 선택된 조건에 따라 필터링되어 최종 리스트로 출력됩니다.
필터 결과는 상단부터 순차적으로 적용되며, 선택 항목에 따라 결과가 달라질 수 있습니다.  
업로드할 엑셀 파일에는 암진료/이식여부 DATA는 'o'로 표시해주십시요.
</div>
